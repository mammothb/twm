...
public static class CommandParser
{
    ...
    public static bool TryParse(
        string line,
        [NotNullWhen(true)] out IpcRequest? request,
    ...
    private static bool TryResize(string[] tokens, out IpcRequest? request, out string? error)
    {
        request = null;
        error = null;
    ...
    private static bool TrySplit(string[] tokens, out IpcRequest? request, out string? error)
    {
        request = null;
        error = null;
        if (tokens.Length != 2)
    ...
    private static bool TryLayout(string[] tokens, out IpcRequest? request, out string? error)
    {
        request = null;
        error = null;
        if (tokens.Length != 2)
        {
            error = $"usage: layout <stacking|tabbed|splith|splitv|toggle-split>";
            return false;
        }
        ...
        switch (tokens[1].ToLowerInvariant())
        {
            case "stacking":
            case "stacked":
                request = new RunCommandRequest(new SetLayoutCommand(Core.Tree.LayoutMode.Stacked));
                return true;
            case "tabbed":
                request = new RunCommandRequest(new SetLayoutCommand(Core.Tree.LayoutMode.Tabbed));
                return true;
            case "splith":
                request = new RunCommandRequest(
                    new SetLayoutCommand(Core.Tree.LayoutMode.SplitHorizontal)
                );
                return true;
            case "splitv":
                request = new RunCommandRequest(
                    new SetLayoutCommand(Core.Tree.LayoutMode.SplitVertical)
                );
                return true;
            case "toggle-split":
            case "toggle":
            ...
            default:
                error =
                    $"invalid layout '{tokens[1]}' (expected stacking|tabbed|splith|splitv|toggle-split)";
                return false;
        }
    }
